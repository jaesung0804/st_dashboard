...
def replay_v2(data: ReplayData, policy: dict, variant='baseline', cost_multiplier=1., fill_delay=1,
    ...
    employee_schedule = policy.get('employee_schedule', {})
    employee_state = {}
    unfilled = 0
    for day, date in enumerate(data.dates):
        ...
        for team, assignments in employee_schedule.get(date_s, {}).items():
            employee_state[team] = assignments
            changed_employees.add(team)
        ...
        for name, b in books.items():
            ...
            if (day-begin-rebalance_phase)%interval != 0 and not (gov_due and name in membership) and name not in changed_employees:
                continue
            if b['pending'] is not None:
                continue
            target_nav = funding[membership[name]]['targets'][name] if gov_due and name in membership else navs[name]
            if name in employee_state:
                weights, score = np.zeros(n), np.zeros(n)
                reasons = []
                for employee in employee_state[name]:
                    ep = policy | employee.get('overrides', {})
                    w, s, why = choose(data, day, name, ep, employee['variant'], b['units'], values[name], navs[name])
                    weights += employee['weight'] * w
                    score += employee['weight'] * np.nan_to_num(s, nan=0.)
                    reasons.append(employee['id']+':'+why)
                reason = 'employee_allocation; '+' | '.join(reasons) if reasons else 'employee_allocation; no eligible employee, retain cash'
            else:
            ...
            if record_details:
                ...
                decisions.append(dict(desk=name, company=membership.get(name,'control'), signal_date=date_s,
                                      ...
                                      positions=positions, employees=employee_state.get(name), pending_at_end=day+fill_delay>=len(data.dates)))
