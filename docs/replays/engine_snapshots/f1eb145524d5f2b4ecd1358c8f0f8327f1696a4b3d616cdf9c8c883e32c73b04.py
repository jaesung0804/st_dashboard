...
def prepare(prices: pd.DataFrame, filings: pd.DataFrame, policy: dict, market: str) -> ReplayData:
    ...
    features["trend200"] = close / close.rolling(200, min_periods=200).mean() - 1
    features["vol63"] = returns.rolling(63, min_periods=40).std() * np.sqrt(252)
