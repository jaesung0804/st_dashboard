...
(() => {
  ...
  function setup() {
    ...
    teamChoices();renderTeams();renderOperations();
  ...
  function renderTeams(){
    ...
    $('teamContext').innerHTML=`운용책임자 ${esc(org.cio_name)} · <a href="${esc(rooms[c])}">회사 회의실 ↗</a> · ${r.case.source==='matched'?'같은 시작 자금과 후보 기준으로 매매 주기를 맞춘 비교입니다.':'장기 표본선정·생존편향을 포함한 연구입니다.'}`;
  ...
  function briefDetails(employee){
    ...
    return `<details><summary>${esc(employee.display_name)} · 승인 요청 전략</summary>${fields.map(([k,label])=>`<p><strong>${label}</strong> · ${esc(b[k])}</p>`).join('')}</details>`;
  }
