...
function heat(value,key){const raw=num(value),good=key==='risk_overheat'?100-raw:raw;const hue=240-(Math.max(0,Math.min(100,Number.isNaN(good)?0:good))*2.4);return `background:hsl(${hue} 80% 91%);color:hsl(${hue} 74% 28%)`;}
