...
const labels=['#','Ticker','Name','Sector','Detail','Close','Up','Up Grade','Down','Down Grade','Expected 6M','Actual 6M'];
...
<style>*{{box-sizing:border-box}}body{{margin:0;font-family:Arial,"Malgun Gothic",sans-serif;background:#f6f8fb;color:#17202a}}header{{background:#101827;color:white;padding:16px 18px}}nav{{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}}nav a{{color:#dbeafe;text-decoration:none;border:1px solid #334155;border-radius:6px;padding:7px 10px;font-size:13px}}main{{max-width:1100px;margin:0 auto;padding:14px}}.panel{{background:white;border:1px solid #d9e2ec;border-radius:8px;padding:12px;overflow:auto}}table{{width:100%;border-collapse:collapse;font-size:12px}}th,td{{border-bottom:1px solid #d9e2ec;padding:8px;text-align:right}}th{{background:#eef3f8}}th:first-child,td:first-child{{text-align:left}}.pos{{color:#047857;font-weight:800}}.neg{{color:#b91c1c;font-weight:800}}@media(max-width:720px){{main{{padding:10px}}table,thead,tbody,tr,th,td{{display:block}}thead{{display:none}}tr{{border:1px solid #d9e2ec;border-radius:8px;margin-bottom:9px;padding:8px}}td{{border:0;display:grid;grid-template-columns:92px 1fr;text-align:left;padding:5px}}td::before{{content:attr(data-label);font-size:11px;color:#64748b;font-weight:800;text-transform:uppercase}}}}</style></head>
