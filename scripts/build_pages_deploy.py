...
<main>
<h1>주식 조기경보 대시보드</h1>
<div class="sub">최근 한 달 신호일을 공개용으로 가볍게 정리한 화면입니다. 이 페이지는 리서치 산출물이며 투자 조언이 아닙니다.</div>
<div class="actions">
...
<a class="button secondary" href="https://github.com/jaesung0804/st_dashboard/actions/workflows/daily-refresh.yml">최신화 실행</a>
</div>
<div class="grid">
...
<body>
<header>
<h1>{label} 조기경보 대시보드</h1>
<div class="sub">{subtitle}. 최근 공개된 한 달 신호일을 보여줍니다.</div>
<nav><a href="../index.html">홈</a><a href="dashboard.html">{label}</a><a href="{other_href}">{other_label}</a><a href="../down_negative_model_comparison/index.html">모델 비교</a><a href="https://github.com/jaesung0804/st_dashboard/actions/workflows/daily-refresh.yml">최신화 실행</a></nav>
</header>
<main>
...
<section class="panel">
<div class="controls"><label>신호일<select id="date"></select></label><label>보기<select id="mode"><option value="final">최종 후보</option><option value="up">상승 후보</option><option value="down">하락 RED</option><option value="all">전체 종목</option></select></label><label>검색<input id="query" placeholder="티커 또는 종목명"></label><label>정렬<select id="sort"><option value="upScore">상승점수</option><option value="downRisk">하락위험</option><option value="growth_profit">성장/수익</option><option value="cash_quality">현금흐름</option><option value="valuation">밸류</option><option value="price_volume">가격/거래</option><option value="risk_overheat">위험/과열</option><option value="ticker">티커</option><option value="name">종목명</option><option value="close">종가</option></select></label><label>페이지당 행<input id="pageSize" type="number" min="10" max="200" value="10"></label></div>
...
const labels=['#','티커','종목명','섹터','대표분야','종가','상승','상승등급','하락','하락등급','대표점수','예상 6개월','실제 6개월'];
...
def stock_html(label: str, other_href: str, other_label: str) -> str:
    return f"""<!doctype html>
<html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{label} 종목 상세</title>
...
<body><header><h1 id="title">{label} 종목 상세</h1><nav><a href="../index.html">홈</a><a href="dashboard.html">{label}</a><a href="{other_href}">{other_label}</a></nav></header><main><section class="panel" id="content">불러오는 중...</section></main>
<script>
...
async function init(){{if(!ticker)throw new Error('티커가 없습니다.');const data=await fetch(`stock_history/${{encodeURIComponent(ticker)}}.json`).then(r=>{{if(!r.ok)throw new Error(`${{r.status}} ${{r.statusText}}`);return r.json();}});document.getElementById('title').textContent=`{label} 종목 상세: ${{data.ticker}} ${{data.name||''}}`;const rows=(data.rows||[]).slice().reverse();document.getElementById('content').innerHTML='<table><thead><tr>'+labels.map(x=>`<th>${{x}}</th>`).join('')+'</tr></thead><tbody>'+rows.map(r=>{{const cells=[esc(r.date),esc(r.close),esc(r.upScore),esc(r.upGrade),esc(r.downRisk),esc(r.downGrade),r.isFinalCandidate?'Y':'',scoreBlock(r),`<span class="${{cls(r.expRet_6m)}}">${{esc(r.expRet_6m||'-')}}</span>`];return '<tr>'+cells.map((c,i)=>`<td data-label="${{labels[i]}}">${{c}}</td>`).join('')+'</tr>';}}).join('')+'</tbody></table>';}}
