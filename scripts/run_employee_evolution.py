        ...
        for team in teams:
            ...
            for employee in staff[team]:
                ...
                if policy.get('supervisor_enabled'):
                    ...
                    proposal=strategy_proposal(team,employee['genome'],policy,date)
                    review=review_promotion(evidence,policy,proposal)
                    events.append(dict(date=date,team=team,employee=employee['id'],action='senior_strategy_review',
                                       supervisor=team+'-LEAD',company_risk_officer=team.split('_')[0]+'-CIO',evidence=evidence,
...
def main():
    ...
    for genome,config in GENOMES.items():
        ...
        if p.get('supervisor_enabled'):
            for team in teams:
                assert validate_proposal(strategy_proposal(team,genome,p,dates[0]))['approved']
